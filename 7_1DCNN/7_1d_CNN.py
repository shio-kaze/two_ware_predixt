...
class CNN_1D(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(self.conv1, nn.ReLU(),self.pool1, self.conv2, nn.ReLU(),self.pool2, self.conv3, nn.ReLU(), self.pool3,
                                 self.conv4, nn.ReLU(),self.pool4, self.conv5, nn.ReLU(),self.pool5, self.conv6, nn.ReLU(), self.pool6)
...
c1_actual = pandas.read_csv('/kaggle/input/labels/c1_wear.csv', index_col=0, header=0,
                          names=['f1', 'f2', 'f3'])
c1_actual = c1_actual.max(axis=1).values.reshape(-1, 1)
c1_label=torch.tensor(c1_actual,dtype=torch.float32)
c1_label=c1_label.cuda()

c4_actual = pandas.read_csv('/kaggle/input/labels/c4_wear.csv', index_col=0, header=0,
                          names=['f1', 'f2', 'f3'])
c4_actual = c4_actual.max(axis=1).values.reshape(-1, 1)
c4_label=torch.tensor(c4_actual,dtype=torch.float32)
c4_label=c4_label.cuda()

c6_actual = pandas.read_csv('/kaggle/input/labels/c6_wear.csv', index_col=0, header=0,
                          names=['f1', 'f2', 'f3'])
c6_actual = c6_actual.max(axis=1).values.reshape(-1, 1)
c6_label=torch.tensor(c6_actual,dtype=torch.float32)
c6_label=c6_label.cuda()
wear=torch.concat([c1_label,c4_label,c6_label],dim=0)
